...
显然，测量工作从树的 `根节点开始`，使用 `深度优先` 完成整个测量工作。
...
* 测量必然从一个`明确自身展示大小` 的 ViewGroup 开始
* 对于一个`子View` -- `A`，当其 `Parent` -- `P` 判断出 `A`
...
* 对于一个Parent，如果它是 `wrap_content`，则需要在子View 的显示大小都确定时，再计算自身大小。
* 只要View-Tree中还 存在 `未确定` 自身 显示大小 的节点。就需要从根节点开始，继续遍历处理测量。
